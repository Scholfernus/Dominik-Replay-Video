...
public class VideoApi {
    ...
    public VideoApi() {
        ...
        videoList.add(new Video(5L, "Something different", "https://youtu.be/qKFP0a63p1U?si=nolRNrbad-BJgxNp"));
    }
    ...
    @PostMapping()
    public ResponseEntity postAddVideo(@RequestBody Video video){
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<List<Video>>removeVideo(@PathVariable Long id){
        Optional<Video> first = videoList.stream().filter(video -> video.getId() == id).findFirst();
        if (first.isPresent()){
            videoList.remove(first.get());
            return ResponseEntity.ok().build();
